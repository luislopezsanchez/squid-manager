...
class AiConfig(Base):
    ...
    # el proveedor elegido para responder -ver ai_service.py-, así que
    # necesitan su propia key, independiente de `api_key` (que es la del
    # `provider` de arriba). Si `provider` ya es "gemini" y esta queda vacía,
    # el servicio reutiliza `api_key` como conveniencia -es la misma cuenta-.
