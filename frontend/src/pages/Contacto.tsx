...
import { IconMail, IconInfo } from '../components/Icons'
...
export default function Contacto() {
  ...
  useEffect(() => {
    api.getUpdateStatus().then(r => setVersion(r.version_actual)).catch(() => {})
  }, [])
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start">
        <div className="card p-6 border border-line-soft space-y-4 w-full md:max-w-2xl">
        ...
        <div className="card p-6 border border-line-soft w-full md:w-72 space-y-3">
          <div className="flex items-center gap-2">
            <span className="stat-icon"><IconInfo /></span>
            <h2 className="text-sm font-semibold text-ink">{traducir("Sobre el proyecto")}</h2>
          </div>
          <div className="text-sm text-ink-3 space-y-1">
            <p className="font-medium text-ink">Luis López Sánchez</p>
            <p>{traducir("Ing. Informático")}</p>
          </div>
          <div className="pt-2 border-t border-line-soft text-sm text-ink-3 space-y-1">
            <p className="font-mono">SquidManager {version ? `v${version}` : ''}</p>
            <p>{traducir("Proyecto 100% cubano.")}</p>
          </div>
        </div>
      </div>
