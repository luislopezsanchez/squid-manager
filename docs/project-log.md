...
| 🟡 Pendiente | Rendimiento del proxy | (2026-09-25) Hacer pruebas de velocidad sin proxy vs. con proxy: usuarios reportan una pérdida notable aun sin ninguna política de tráfico (delay pools, límites) creada — hay que descartar si el costo viene del propio Squid (SSL bump, antivirus/ICAP si está activo) o de otra parte |
| 🟡 Pendiente | Plataforma / operaciones | (2026-09-25) Al aplicar cambios con ACLs de archivo grandes cargadas, la reconfiguración de Squid corta la conexión de los clientes mientras dura — se sugirió un helper aparte (proceso separado) para procesar/cargar esos archivos grandes en vez de bloquear el `reconfigure` principal |
| 🟡 Pendiente | Plataforma / operaciones | (2026-09-25) Borrar un mes histórico desde el panel (`delete_historical_month.sh` vía sudoers) solo se implementó y verificó para instalación nativa — en Docker el backend no tiene sudo ni acceso de escritura al volumen `squid-logs`; hoy devuelve un error claro en vez de romper, pero falta un mecanismo real (¿`docker exec` contra el contenedor de Squid, que sí escribe ahí?), sin verificar por falta de un entorno Docker disponible |
| 🟡 Pendiente | Análisis de datos | (2026-09-25) "Personalizado..." (rango de fechas libre) solo funciona de punta a punta en Actividad de red. Se encontró un bug real durante una revisión a fondo de la sección Análisis: Tendencias, Panorama y Rendimiento y errores compartían el mismo `SelectorVentana` y mostraban la opción igual, pero al elegirla no aparecía ningún selector de fechas y la ventana quedaba silenciosamente en "últimas 1000 peticiones" sin avisar -un callejón sin salida en el menú. Se corrigió ocultando la opción en esas tres páginas (no ofrecerla es mejor que ofrecerla rota); implementar el rango libre de verdad ahí (backend: `_read_rango` ya existe y lo usa Actividad de red, faltaría cablearlo a `get_tendencia_trafico`/`get_volumen_por_periodo`/`get_latencia`/`get_http_errors`) queda pendiente, ya estaba fuera de alcance desde que se hizo Actividad de red |
| 🟡 Pendiente | Plataforma / operaciones | (2026-09-25/26) Verificando Proxy padre/hijo en cascada (3 SquidManager reales, `.88 → .92 → .35`) apareció dos veces, en dos VMs distintas, un crash de Squid 6.14 ajeno a SquidManager: `FATAL: assertion failed: store_swapout.cc:276` justo al arrancar el helper de autenticación básica para la PRIMERA petición autenticada que recibe ese Squid desde que arrancó, seguido de una cascada de `Ipc::Mem::Segment::open failed to shm_open(/squid-tls_session_cache.shm)` en los reintentos internos hasta que Squid se rinde ("will not be restarted for 3600 seconds"). No es determinístico (un reinicio posterior del mismo Squid no volvió a crashear con la misma petición) y no depende de nada que este proyecto escriba en squid.conf -parece un bug de Squid 6.14 en el arranque perezoso del helper de auth combinado con el subsistema de store. El botón "Iniciar Squid" del dashboard (agregado hoy mismo) sí recupera el servicio sin pasos manuales extra. Pendiente: si vuelve a aparecer, reportarlo aguas arriba a Squid con este mensaje exacto, o probar fijar una versión de Squid distinta en estas VMs de prueba |
