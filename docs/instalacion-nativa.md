...
> ⚠️ **Ubuntu 26.04 todavía no está soportado.** Trae Python 3.14 por
> defecto, y `psycopg[binary]` y `pydantic-core` no tienen binarios
> compilados para esa versión (`pydantic-core` ni siquiera compila desde
> código fuente: su toolchain de Rust —PyO3 0.22— no soporta Python 3.14
> en absoluto). El paso 8 del instalador falla con `ERROR: Could not find
> a version that satisfies the requirement psycopg-binary==3.2.3`. Usa
> Ubuntu 24.04 o Debian 12 hasta que el ecosistema Python se ponga al día.
...
instalador, que se baja fresco cada vez, hace un backup de la base, trae el
código nuevo de forma segura y recién ahí corre el `install-nativo.sh` ya
actualizado (que es quien preserva tu configuración y repara lo que un
`git pull` a secas nunca vuelve a tocar: paquete de PostgreSQL, drop-in de
...
incluido por qué no alcanza con re-correr `install-nativo.sh` a mano—, el
comando manual equivalente y qué hacer si algo falla a mitad de camino.
...
keytab correcto (ver [docs/kerberos.md](kerberos.md)). El `daemon-reload` solo
recarga la definición del servicio; hace falta además el `restart` porque las
variables de entorno de un proceso se fijan al arrancarlo, no se pueden
inyectar en uno que ya está corriendo.
